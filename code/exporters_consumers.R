...
#### [2] look at exporters/consumers where source country is country of interest ####
...
exporter_summary_countries <- exports_from_countries %>%
  group_by(SourceCountry,ExporterCountry, Nutrient) %>%
  summarise(
...
consumer_summary_countries <- exports_from_countries %>%
  ...
  ungroup() %>%
  rename(Country=ConsumerCountry) %>%
  mutate(Role="Consumer")
...
role_ranked_countries <- role_df_countries %>%
  mutate(Role = factor(Role, levels = c("Exporter", "Consumer"))) %>%
  group_by(Country,Role) %>%
  mutate(RNIsum = sum(TotalRNIAmount)) %>% ungroup() %>%
  group_by(SourceCountry,Role) %>%  # group by Role to apply slice within each group
  arrange(desc(RNIsum)) %>%
...
ggplot(role_ranked_countries, 
       aes(x = reorder_within(Country, -TotalRNIAmount,interaction(SourceCountry,Role)), 
           y = TotalRNIAmount, fill = Nutrient)) +
  facet_wrap(~SourceCountry * Role,scales = "free")+
  geom_bar(stat = "identity") +
  scale_x_reordered()+
  labs(x = NULL,
       y = "Total RNIs", 
       title = "Countries Receiving Nutrients From Source ") +
  theme_minimal() + 
